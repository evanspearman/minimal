...
- **`patches`**: `{ source, dest }` rows copying files into the session.
  ...
  on the host, typically inside the repo, and may be a glob or a directory
  (a directory takes the whole tree beneath it). The one expansion a loadout
  has and a project does not is `$LOADOUT_ROOT`, which names a loadout's own
